...
#include "CPP_SpawnBox_Murdock.h"
...
// Sets default values
ACPP_SpawnBox_Murdock::ACPP_SpawnBox_Murdock()
{
...
void ACPP_SpawnBox_Murdock::BeginPlay()
{
	Super::BeginPlay();
	CreateTargetMesh();
	
	GetWorld()->GetTimerManager().SetTimer(SpawnTimerHandle,this,&ACPP_SpawnBox_Murdock::OnSpawnTimerElapsed,SpawnInterval,true);	
}

// Called every frame
void ACPP_SpawnBox_Murdock::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

FVector ACPP_SpawnBox_Murdock::GetTargetLocation_() const
{
...
void ACPP_SpawnBox_Murdock::SpawnEnemy()
{
	if(HasAuthority())
	{
	 if (EnemyClass)
     {
        ...
        //ACPP_Dorongo* 
		SpawnedEnemy = GetWorld()->SpawnActor<ACPP_Murdock>(EnemyClass, SpawnLocation, SpawnRotation, SpawnParams);

		if (SpawnedEnemy)
        {
			if(AIControllerClass)
			{
            	AAIController_Murdock* AIController = GetWorld()->SpawnActor<AAIController_Murdock>(AIControllerClass);
            	if (AIController)
            	{
...
void ACPP_SpawnBox_Murdock::OnSpawnTimerElapsed()
{
...
bool ACPP_SpawnBox_Murdock::IsEnemyValid()
{    
    return SpawnedEnemy && !SpawnedEnemy->IsPendingKill();
}

void ACPP_SpawnBox_Murdock::CreateTargetMesh()
{
	if (TargetMeshComponent)
    {
