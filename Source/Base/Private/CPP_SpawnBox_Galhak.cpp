...
// Sets default values
ACPP_SpawnBox_Galhak::ACPP_SpawnBox_Galhak()
{
...
void ACPP_SpawnBox_Galhak::BeginPlay()
{
	Super::BeginPlay();
	CreateTargetMesh();
	
	GetWorld()->GetTimerManager().SetTimer(SpawnTimerHandle,this,&ACPP_SpawnBox_Galhak::OnSpawnTimerElapsed,SpawnInterval,true);
}

// Called every frame
void ACPP_SpawnBox_Galhak::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

FVector ACPP_SpawnBox_Galhak::GetTargetLocation_() const
{
...
void ACPP_SpawnBox_Galhak::SpawnEnemy()
{
	if(HasAuthority())
	{
	 if (EnemyClass)
     {
        ...
        //ACPP_Dorongo* 
		SpawnedEnemy = GetWorld()->SpawnActor<ACPP_Galhak>(EnemyClass, SpawnLocation, SpawnRotation, SpawnParams);

		if (SpawnedEnemy)
        {
			if(AIControllerClass)
			{
            	AAIController_Galhak* AIController = GetWorld()->SpawnActor<AAIController_Galhak>(AIControllerClass);
            	if (AIController)
            	{
...
void ACPP_SpawnBox_Galhak::OnSpawnTimerElapsed()
{
...
bool ACPP_SpawnBox_Galhak::IsEnemyValid()
{    
    return SpawnedEnemy && !SpawnedEnemy->IsPendingKill();
}

void ACPP_SpawnBox_Galhak::CreateTargetMesh()
{
	if (TargetMeshComponent)
    {
