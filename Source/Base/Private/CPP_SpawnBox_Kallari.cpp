...
#include "CPP_SpawnBox_Kallari.h"
...
// Sets default values
ACPP_SpawnBox_Kallari::ACPP_SpawnBox_Kallari()
{
...
void ACPP_SpawnBox_Kallari::BeginPlay()
{
	Super::BeginPlay();
	CreateTargetMesh();
	
	GetWorld()->GetTimerManager().SetTimer(SpawnTimerHandle,this,&ACPP_SpawnBox_Kallari::OnSpawnTimerElapsed,SpawnInterval,true);
}

// Called every frame
void ACPP_SpawnBox_Kallari::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

FVector ACPP_SpawnBox_Kallari::GetTargetLocation_() const
{
...
void ACPP_SpawnBox_Kallari::SpawnEnemy()
{
	if(HasAuthority())
	{
	 if (EnemyClass)
     {
        ...
        //ACPP_Dorongo* 
		SpawnedEnemy = GetWorld()->SpawnActor<ACPP_Kallari>(EnemyClass, SpawnLocation, SpawnRotation, SpawnParams);

		if (SpawnedEnemy)
        {
			if(AIControllerClass)
			{
            	AAIController_Kallari* AIController = GetWorld()->SpawnActor<AAIController_Kallari>(AIControllerClass);
            	if (AIController)
            	{
...
void ACPP_SpawnBox_Kallari::OnSpawnTimerElapsed()
{
...
bool ACPP_SpawnBox_Kallari::IsEnemyValid()
{    
    return SpawnedEnemy && !SpawnedEnemy->IsPendingKill();
}

void ACPP_SpawnBox_Kallari::CreateTargetMesh()
{
	if (TargetMeshComponent)
    {
