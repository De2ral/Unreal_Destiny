...
#include "CPP_SpawnBox_Exploder.h"
...
// Sets default values
ACPP_SpawnBox_Exploder::ACPP_SpawnBox_Exploder()
{
...
void ACPP_SpawnBox_Exploder::BeginPlay()
{
	Super::BeginPlay();
	CreateTargetMesh();
	
	GetWorld()->GetTimerManager().SetTimer(SpawnTimerHandle,this,&ACPP_SpawnBox_Exploder::OnSpawnTimerElapsed,SpawnInterval,true);
}

// Called every frame
void ACPP_SpawnBox_Exploder::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

FVector ACPP_SpawnBox_Exploder::GetTargetLocation_() const
{
...
void ACPP_SpawnBox_Exploder::SpawnEnemy()
{
	if(HasAuthority())
	{
	 if (EnemyClass)
     {
		...
		SpawnedEnemy = GetWorld()->SpawnActor<ACPP_Exploder>(EnemyClass, SpawnLocation, SpawnRotation, SpawnParams);

		if (SpawnedEnemy)
        {
			if(AIControllerClass)
			{
            	AAIController_Exploder* AIController = GetWorld()->SpawnActor<AAIController_Exploder>(AIControllerClass);
            	if (AIController)
            	{
...
void ACPP_SpawnBox_Exploder::OnSpawnTimerElapsed()
{
...
bool ACPP_SpawnBox_Exploder::IsEnemyValid()
{    
    return SpawnedEnemy && !SpawnedEnemy->IsPendingKill();
}

void ACPP_SpawnBox_Exploder::CreateTargetMesh()
{
	if (TargetMeshComponent)
    {
